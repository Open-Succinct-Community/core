...
public abstract class XMLSerializationHelper {
	  ...
	  public static void serialize(Node target, File file, boolean indent){
		  try {
			  serialize(target, new FileWriter(file), true);
		  }catch (Exception ex){
			  throw new RuntimeException(ex);
		  }
	  }
	  public static void serialize(Node target, Writer wr, boolean indent){
		  try {
			  OutputFormat format = new OutputFormat();
			  format.setIndenting(indent);
			  XMLSerializer serializer = new XMLSerializer(wr, format);
			  if (target instanceof Document) {
				  serializer.serialize((Document)target);
